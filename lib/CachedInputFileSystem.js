...
const runCallbacks = (callbacks, err, result) => {
	// console.log({line:44, callback0: callbacks[0].toString()})
	if (callbacks.length === 1) {
		// console.log("\nBelow is callbacks index 0's strings")
		// console.log(callbacks[0].toString())
		// console.log("\n")
...
class CacheBackend {
	...
	provide(path, options, callback) {
		...
		if (cacheEntry !== undefined) {
			console.log({ line: 251, strPath, cacheEntry });
		...
		(this._provider).call(
			 ...
			 */
			(err, result) => {
				console.log({ strPath, line: 277 });
				console.log({
					line: 278,
					currentOperations: this._activeAsyncOperations
				});
				this._activeAsyncOperations.delete(strPath);
				this._storeResult(strPath, err, result);
				console.log({ line: 281, file: "cached", strPath });
				console.log({ line: 282, file: "cached", data: this._data });

				// Enter async mode if not yet done
				this._enterAsyncMode();

				/// Todo: when is a function stored to runCallbacks??
				runCallbacks(
	...
	provideSync(path, options) {
		...
		// Check in cache
		let cacheEntry = this._data.get(strPath);
		console.log({ cacheEntry, line: 320 });
		if (cacheEntry !== undefined) {
	...
	_storeResult(path, err, result) {
		...
		console.log({ line: 437, levels: this._levels });
		const level = this._levels[this._currentLevel];
		console.log({ line: 438, level, log: "level in storeResult" });
	...
	_runDecays() {
		console.log({
			line: 472,
			nextDecay: this._nextDecay,
			Date: Date.now(),
			resultOfIf: this._nextDecay <= Date.now(),
