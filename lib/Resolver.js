...
class Resolver {
		...
		if (resolveContext.log) {
			...
			return this.doResolve(
				...
				(err, result) => {
					...
					if (result) {
						console.log("result at 628 in Resolver.js", result);
	...
	doResolve(hook, request, message, resolveContext, callback) {
		console.log("doResolve at 671 in Resolver.js...", hook.name);
		const stackEntry = Resolver.createStackEntry(hook, request);
		// console.log("stackEntry at 673 in Resolver.js...", stackEntry);
		...
		/// After resolveAsModule hook, the stack looks like this:
		/// undescribedRawFile --> rawFile --> file --> finalFile --> ... --> undescribedResolveInPackage --> resolveInPackage --> relative --> describedRelative --> rawFile ....
		// console.log("newStack at 702 in Resolver.js...", newStack);
		// console.log("hook at 703 in Resolver.js...", hook.name);
		/// resolveStep is SyncHook!!!
		if (hook.name === "resolveAsModule") {
			/// I dont know what this line do
			/// --> Maybe this line calls just only resolveStep. Todo: Search what resolveStep does
		...
		if (hook.isUsed()) {
			...
			// if (hook.name==="resolve") {
			// 	console.log("\nhook.callAsync at 713 in Resolver.js request... ", request);
			// 	console.log("hook.callAsync at 713 in Resolver.js innerContext... ", innerContext);
			// 	console.log("\n")
			// }
			/// This will call the hook for each time!! ... in my guess
			return hook.callAsync(request, innerContext, (err, result) => {
				if (err) return callback(err);
				// 	console.log(`\nhook.name is... ${hook.name}`)
				// console.log("hook.taps", hook.taps.map(t => t.name))
				// 	console.log("result of hook.callAsync at 743 in Resolver.js...", result);
				// 	console.log("\n")
				if (result) {
					/// resolveAsModule hook actually resolved `acorn` to `node_modules/acorn/dist/acorn.js`, because the result appeared for the first time when it was called.
